...
class Timer {
 public:
   ...
   * @param func_name
   */
  template <class F> static void Evaluate(F&& func, const std::string& func_name) {
  ...
  /// dump to a log file
  static void DumpIntoFile(const std::string& file_name = "timer.log") {
    ...
    for (size_t i = 0; i < max_length; ++i) {
      for (const auto& iter : records_) {
        if (i < iter.second.time_usage_in_ms_.size()) {
          oss << iter.second.time_usage_in_ms_[i] << ",";
        } else {
          oss << ",";
        }
      }
    ...
    if (!ofs.is_open()) {
      LOG(ERROR) << "Failed to open file: " << file_name << std::endl;
      return;
    } else {
      LOG(INFO) << "Dump Time Records into file: " << file_name << std::endl;
    }
